...
describe(`Test 6`, () => {
  ...
  test.skip(`It works with queries with 0 results`, async () => {
    const result = await scrapeGoogle({query: 'ksjdfhklsfjhalskflaskdfaksjfkalsjfkasjfkjsafhjka'});
    expect(result.length).toBe(0);
  }, 20000);

  test.skip(`It works with queries with less than 10 results`, async () => {
    const result = await scrapeGoogle({query: 'site:conio.com multisig'});
    expect(result.length).toBeLessThanOrEqual(10);
  }, 20000);

  test.skip(`It works with queries with more a number of results included between 10 and 30`, async () => {
